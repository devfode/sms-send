...
/**
 * WorkManager worker that sends an email with SMS content via SMTP.
 * Falls back to SmtpDefaults if no config is found in ConfigStore.
 */
class SendEmailWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            // Try ConfigStore first, fall back to hardcoded defaults
    ...
    private fun formatEmailBody(from: String, body: String, timestamp: Long, slotId: Int?): String {
        ...
        val slotInfo = slotId?.let { " (SIM slot: $it)" } ?: ""

        return """
From: $from
Time: $formattedDate
Device: $deviceInfo$slotInfo
